   ...
   #如果不能运行就把这个放到use_gpu后面，或者前面
    ...
    while n < meta_step:
        ...
        if attention == False:
            ...
            attn_meta_model.load_state_dict(attn_model.state_dict())
        b = 1    
        training = train_keys.copy()
        while(len(training)!=0):
            choose_list = random.sample(training,bsz)
            print('Batch------',b)
            print("choose video list:",choose_list)
            b = b+1
            training = list(set(training)-set(choose_list))
            k = 0
            epis_rewards = {key: [] for key in train_keys}
            
            for epoch in range(start_epoch, args.max_epoch):
                #print("----------------------training----------------------") if k==0 else print("----------------------training-shuffle---------------------")
                k+1
                shuffle(choose_list)
                #epi = 0
                loss_5 = 0 
                reward_writers = [] # 记录reward的变化
                for idx in choose_list:
                    ...
                    loss_5 += cost_1.item()
                    epis_rewards[key].append(reward_1.item())
                    reward_writers.append(reward_1.item())
                    if attention == False:
                    ...
                    baselines[key] = 0.9 * baselines[key] + 0.1 * np.mean(epis_rewards[key]) # 更新reward             
                reward_writers = np.mean(reward_writers)
                total_loss += loss_5 #综合损失
                total_times += 1
                print("epoch {}/{} meta_epoch{}/{} reward{} loss{}".format(epoch+1,args.max_epoch,n+1,meta_step,reward_writers,loss_5))
            #print("the cost for 5 videos",loss_5)
            #print("the reward for 5 videos:",reward_writers)
        if attention == True:
            ...
            cost_2.backward()
            optimizer.step()     
        #epoch_reward = np.mean(reward_writers)
        #print("epoch {}/{} meta_epoch {}/{}\t reward {}\t loss {} ".format(epoch+1, args.max_epoch,n+1,meta_step, epoch_reward,cost_2.item()))
        n=n+1
